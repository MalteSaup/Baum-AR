...
public class BaumScript : MonoBehaviour
{
	...
	public void fallingLeavesStart(int angle){				//Startet Animation der Blätter zum abfallen
		...
		if((angle - 15) % 90 <= 60){
			for(int i = 0; i <= (angle - 15) % 90; i++){
				...
				if(!herbstAudioPlaying){			//Startet Windstoß Sound wenn er noch nicht gespielt wurde
					herbstAudioPlaying = true;
					backgroundNoise.PlayOneShot(jahreszeiten[4], 0.05f);	//Startet Sound
					StartCoroutine(soundLeaves());				//Startet Coroutine um zu wissen wann Sound zuende
				}
			}
		}
	}
	...
	public IEnumerator soundLeaves(){									//Coroutine damit Sound nicht mehrere Male gleichzeitig spielt
		yield return new WaitForSeconds(3f);								//Sound dauert ~2.8s und deaktivierung erfolgt nach 3s
		herbstAudioPlaying = false;
	}


	public void changeVolumne(int angle, AudioClip jahreszeit){				//Ändert Sound Lautstärke und den gespielten Sound 
	...
	public void audioChangeCall(int angle, AudioClip actual, AudioClip next){		//Ruft changeVolume mit den benötigten Sound auf
		...
		if(tracked){
			...
			if(check){
				...
				if(angle >= 15 && angle < 75){   //Frühling zu Sommer
					audioChangeCall(angle, jahreszeiten[0], jahreszeiten[1]);		//Update für Sound Lautstärke und Sound
